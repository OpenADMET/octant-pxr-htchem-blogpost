...
Together, the three HTChem libraries, including "semi-purified" crudes, expand the publicly-available PXR screening dataset and are intended to facilitate further exploration of PXR SAR across related scaffolds. All raw data for the libraries and experiments described in this post—including CAD peak areas, CAD-derived yields, pEC50 values, and yield-corrected pEC50 calculations with error estimates and confidence intervals—are available in the [accompanying GitHub repository](https://github.com/OpenADMET/octant-pxr-htchem-blogpost).

::: {.callout-note}
## Accessing the Data on HuggingFace

The crude and semi-pure HTChem library data released with this post are also available on HuggingFace at [`openadmet/pxr-challenge-train-test`](https://huggingface.co/datasets/openadmet/pxr-challenge-train-test). They can be loaded with the [`datasets`](https://huggingface.co/docs/datasets) library:
...
For the crude libraries, key columns include `SMILES`, `OCNT_ID`, `Corrected Crude pEC50 (log)`, `Corrected Crude pEC50 ±95% CI (log)`, `Crude Product Yield (%)`, and `Crude Peak Area (pA*min)`. The semi-pure dataset shares the same schema, with yield and pEC50 values reflecting the semi-purified sample format.
:::
